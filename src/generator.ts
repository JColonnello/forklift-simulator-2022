...
function mirrorVertices(vertices: Vector2[], axis: Vector2): Vector2[] {
  return [
    ...
    ...vertices
      .slice(0, -1)
      .map((v) =>
        new Vector3(v.x, v.y).reflect(new Vector3(axis.x, axis.y).normalize())
      )
        ...
        generate = () => {
          ...
          for (let i = 0; i < 4; i++) {
            const v = vectors.map((v) =>
              v.clone().rotateAround(center, (Math.PI / 2) * i)
            );
...
export class SweepModelGenerator extends ModelGenerator {
  ...
  constructor(shape: BaseShape, torsionAngle: number, height: number) {
    super();
    if (!shape.canExtrude) throw new TypeError("Cannot extrude this shape");
    this.shape = shape;
...
export class RevolveModelGenerator extends ModelGenerator {
  ...
  constructor(shape: BaseShape) {
    super();
    if (!shape.canRevolve) throw new TypeError("Cannot revolve this shape");
    this.shape = shape;
  }
